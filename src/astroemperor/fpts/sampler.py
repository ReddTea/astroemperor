...
class Sampler(object):
    ...
    def log_evidence_estimate(self, logls=None, fburnin=0.1, coef=3):
        """
        ...
        for i in range(logls.shape[0]):
            loglrow = logls[i].flatten(order='F')[int(istart*logls.shape[1]):]
            mask = (np.median(loglrow) + coef*np.std(loglrow) > loglrow) & (loglrow > np.median(loglrow) - coef*np.std(loglrow))
            #mask = True
            print(len(loglrow[mask]))
            loglrow0 = np.mean(loglrow[mask])
            #loglrow0 = np.mean(loglrow)
            loglsm = np.append(loglsm, loglrow0)

        #mean_logls = np.mean(np.mean(logls, axis=1)[:, istart:], axis=1)

        return util.thermodynamic_integration_log_evidence(self._betas, loglsm)
