...
def ensure_dir(name, loc='', k=0, first=True):
    ...
    os.makedirs(f'{dr}/plots/betas')
    os.makedirs(f'{dr}/plots/histograms')
    os.makedirs(f'{dr}/plots/GMEstimates')
    os.makedirs(f'{dr}/plots/arviz')
    os.makedirs(f'{dr}/plots/arviz/cornerplots')
    os.makedirs(f'{dr}/plots/arviz/traces')
    os.makedirs(f'{dr}/plots/arviz/normed_posteriors')
    os.makedirs(f'{dr}/plots/arviz/density_intervals')
...
def adjust_table_tex(to_tab0, rounder=3):
    for j in range(len(to_tab0)):
        ...
        elif (type(to_tab0[j][0]) == np.float64 or
              type(to_tab0[j][0]) == list or
              type(to_tab0[j][0]) == np.ndarray):
        ...
        if len(self.AM_sets) > 0:
            for file in self.AM_sets:
                ...
                if identifier == 'hipgaia.hg123':
                    self.df_hipgaia = pd.read_csv(ff, sep=r'\s+')

                if identifier == 'hip2.abs':
                    self.df_hip = pd.read_csv(ff, sep=r'\s+')
    ...
    def mk_AM(self):
        ...
        for file in m['filenames']:
            ...
            if identifier == 'gost.csv':
                ...
                m['df_hipgaia'] = pd.read_csv(ff, sep=r'\s+')
