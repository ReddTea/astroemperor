...
class Model(Model):
    parameter_names = ("T0", "radius", "log_sigma2")
...
def init_george(x, yerr):
    gp = george.GP(2. * kernels.Matern32Kernel(2.))
    gp.compute(x, yerr)
    return gp

def lnlike_gp(param, x, y, yerr, gp):
    k_a, k_r = param[-2], 10.**param[-1]
    gp.set_parameter_vector(np.array([k_a, k_r]))
    return gp.lnlikelihood(y - Model(param[:-2], x))  # T0, r, param[:-2]
...
gp_kernel = init_george(xx, yy_error)

data = (xx, yy, yy_error)

p0, ndim = give_initial_pt(nwalkers)
sampler = emcee.PTSampler(ntemp, nwalkers, ndim, lnlike_gp, lnprior, loglargs=[xx, yy, yy_error, gp_kernel])

print("Running burn-in")

#pbar = tqdm(total=burn_out1)
