...
# creates n new points; n=madness
...
def lnlike_gp(param, x, y, yerr, gp):
    try:
        ...
        return gp.log_likelihood(y - Model(param[:-2], x))
    except:
        return -np.inf
...
def init_celerite(x, yerr):
    kernel = terms.Matern32Term(log_rho=2., log_sigma=2.)
...
cel_gp = init_celerite(xx, yy_error)

p0, ndim = give_initial_pt(nwalkers)
sampler = emcee.PTSampler(ntemp, nwalkers, ndim, lnlike_gp, lnprior, loglargs=[xx, yy, yy_error, cel_gp])
